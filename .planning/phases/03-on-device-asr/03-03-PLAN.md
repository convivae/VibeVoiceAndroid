...
Purpose: Handle on-demand model download (per D-05: APK ~20-30MB, model ~400MB downloaded on first use).
...
# - D-05: APK ~20-30MB, model (~400MB) downloaded on first use
...
class ModelDownloadInProgress extends ModelDownloadState {
  ...
  String get sizeText => '${(totalBytes / (1024 * 1024)).toStringAsFixed(1)} MB';
}
      ...
      onProgress: (progress) {
        if (progress >= 0) {
          ...
          state = ModelDownloadInProgress(
            progress: progress,
            receivedBytes: (progress * 400 * 1024 * 1024).toInt(),  // Estimate 400MB
  ...
  Widget _buildProgress(BuildContext context, ModelDownloadInProgress state) {
    return Column(
      children: [
        ...
        const SizedBox(height: 16),
        Text(
          '${state.progressPercent} (${state.receivedBytes ~/ (1024 * 1024)} / ${state.sizeText})',
